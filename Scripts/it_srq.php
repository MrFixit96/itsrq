  ...
  if (!isset($_REQUEST['action'])){//checks action variable and kicks it to the post_entry function if its empty
    ...
    switch ($_REQUEST['action']){   //checks action variable and kicks it to the correct function
      ...
      case ($_REQUEST['action']=="Submit"):
           ...
           break;
      case ($_REQUEST['action']=="View" || $_REQUEST['action']=="View Open"):
           ...
           break;
      case ($_REQUEST['action']=="View Tasks"):
      ...
      case ($_REQUEST['action']=="Reports"):
           report_panel($owner);
           break;
      case ($_REQUEST['action']=="Send Email"):
           Email_panel($owner, $srq_user);
           break;
      case ($_REQUEST['action']=="Manual Mail"):
           Manual_Mailer($owner, $srq_user);
           break;
      ...
      case ($_REQUEST['action']=="Update"):
           ...
           break;
      case ($_REQUEST['action']=="view_request"):
...
function post_entry($db_connection, $owner, $srq_user, $srq_user_email, $name, $email, $assetNumber, $priorityLevel, $status, $timeStart, $errorType, $description, $OS, $admin_email_notification_subject, $admin_email_list, $global_owner_email, $owner_email_beg_url, $owner, $form_options_windows_version, $form_options_error_type, $form_options_priority_level, $iTask=null){
  //echo "begin teh fuhnestt";

  //print_r($errors);
  
  $errors = array();
  //$happytimefun = count($errors);
  //echo "happyfuntime:  $happytimefun <br /><br />";

  if(isset($_REQUEST["formloaded"])){
    $fun = validate_post_data($name, &$email, $assetNumber, $status, $priorityLevel, $errorType, $description, $OS, $assigned, $timeStart, $timeStop);
    //echo "<br /><br />fun:  ";
    //print_r($fun);
...
########Submit data to the database via sql                                
    ...
    #######Send an email alert with request contents
...
########Submit data to the database via sql
    ...
    $stmt = mysqli_prepare($db_connection, $script);
    //mysqli_stmt_bind_param($stmt, 'ssssssssssss', $name, $email, $assetNumber, $priorityLevel, $status, $timeStart, $errorType, $description, $OS, $assigned, $timeStop, $resolution);
	...
	$post_ID = $RegID;
	#echo "email action: " . $email_action . "\n post_ID: " . $post_ID . "\n regID: " . $RegID . "\n fun";
...
############################## Display the requests
    ...
    p_beg();
    table_beg();
/*         colgroup_beg();     //colgroup can be used to format columns
           col("id_request");
           col("id_name");
           col("id_email");
           col("id_assetNumber");
...
function admin_panel($owner){
    ...
    anchor("it_srq.php?action=Post", "Post a Request", "clamenuitem");
    anchor("it_srq.php?action=View", "View Open Requests", "clamenuitem");
    anchor("../../?section=51", "Back To IT Request System Home", "clamenuitem");
    div_end();
    p_end();
    hr;
    p_beg();
