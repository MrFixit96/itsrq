...
Const L_CannotStart_Message            = "%1: Server cannot be started in its current state(%2 server is %3)"
Const L_CannotStop_Message             = "Server cannot be stopped in its current state(%1 server is %2)"
Const L_CannotPause_Message            = "Server cannot be paused in its current state(%1 server is %2)"
...
Const L_Admin_ErrorMessage             = "You cannot run this command because you are not an administrator on the server you are trying to configure."
...
Const L_Help_HELP_General21a_Text= "                        /SetADProp  Sets FTPRoot and FTPDir values in Active Directory."
Const L_Help_HELP_General21b_Text= "                        /GetADProp  Queries FTPRoot and FTPDir values from Active Directory."
...
Const L_Help_HELP_GetADProp02_Text   = "Syntax: IIsFtp [/s <server> [/u <username> [/p <password>]]] /GetADProp"
Const L_Help_HELP_GetADProp02p1_Text = "        <UserID> FTPDir | FTPRoot"
Const L_Help_HELP_GetADProp10_Text   = "<UserID>                The logon id of the Active Directory user to grab the property from."
...
Const L_Help_HELP_SetADProp02_Text   = "Syntax: IIsFtp [/s <server> [/u <username> [/p <password>]]] /SetADProp"
Const L_Help_HELP_SetADProp02p1_Text = "        <UserID> FTPDir | FTPRoot <propval>"
Const L_Help_HELP_SetADProp10_Text   = "<UserID>                The logon id of the Active Directory user to set the property at."
...
Const L_Help_HELP_Create03_Text   = "        [/isolation <isomode> [/ADDomain <domain> /ADAdmin <admin> /ADPass <password>]]"
...
Const L_Help_HELP_Create17_Text   = "/AD*                    Set ActiveDirectory credentials for AD isolation mode."
                ...
                Select Case newStatus
                    Case SERVER_STARTED
                        ...
                        Else 
                            If (Server.ServerState = SERVER_PAUSED) Then
                                ...
                                Server.Continue
                            Else
                                oCmdLib.vbPrintf L_CannotStart_Message, Array(strSiteName, strSiteName, SiteStatus(Server.ServerState))
                                bNonFatalError = True
                            End If
                        End If
                    ...
                    Case SERVER_STOPPED
                        If (Server.ServerState = SERVER_STARTED) Then
                            ...
                            Server.Stop
                        Else
                            oCmdLib.vbPrintf L_CannotStop_Message, Array(strSiteName, SiteStatus(Server.ServerState))
                            bNonFatalError = True
                        End If
                    ...
                    Case SERVER_PAUSED
                        If (Server.ServerState = SERVER_STARTED) Then
                            ...
                            Server.Pause
                        Else
                            oCmdLib.vbPrintf L_CannotPause_Message, Array(strSiteName, SiteStatus(Server.ServerState))
                            bNonFatalError = True
                        End If
...
Function DeleteFtpSite(aArgs)
    ...
    For Each strSiteName in aSites
        ...
        oCmdLib.vbPrintf L_HasBeen_Message, Array(strSite, UCase(SiteStatus(8)))
    Next
...
Function CreateFtpSite(strRoot, strName, strPort, strIP, bDontStart, strISO, strADDomain, strADAdmin, strADPass)
    ...
    If (strISO = "") Then strISO = L_No_ISO
