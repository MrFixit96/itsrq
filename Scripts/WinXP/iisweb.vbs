...
Const L_CannotStop_Message      = "%1: Server cannot be stopped in its current state(%2 server is %3)"
Const L_CannotPause_Message     = "%1: Server cannot be paused in its current state(%2 server is %3)"
            ...
            Else
                ...
                Case SERVER_STOPPED
                    If (Server.ServerState = SERVER_STARTED) Then
                        ...
                        Server.Stop
                    Else
                        oCmdLib.vbPrintf L_CannotStop_Message, Array(strSiteName, strSiteName, SiteStatus(Server.ServerState))
                        bNonFatalError = True
                    End If
                ...
                Case SERVER_PAUSED
                    If (Server.ServerState = SERVER_STARTED) Then
                        ...
                        Server.Pause
                    Else
                        oCmdLib.vbPrintf L_CannotPause_Message, Array(strSiteName, strSiteName, SiteStatus(Server.ServerState))
                        bNonFatalError = True
                    End If
    ...
    For Each Server in Servers
        ...
        If (IsArray(bindings)) Then
            For i = LBound(bindings) to UBound(bindings)
                ...
                Else
                    line = Space(54) & strIP & Space(16 - Len(strIP)) & strPort & Space(6 - Len(strPort)) & strHost
                End If
