  ...
  if (!isset($_REQUEST['action'])){//checks action variable and kicks it to the post_entry function if its empty
    ...
    switch ($_REQUEST['action']){   //checks action variable and kicks it to the correct function
      ...
      case ($_REQUEST['action']=="Update"): //takes changes from change_entry and posts them to the database
           update_entry($db_connection, $owner, $name, $email, $assetNumber, $priorityLevel, $status, $timeStart, $errorType, $description, $OS, $assigned, $timeStop, $resolution, $RegID, $email_action, $post_ID, $admin_email_list,  $owner_email_beg_url, $admin_email_notification_subject, $global_owner_email);
           break;
...
function validate_post_data($name, &$email, $assetNumber, $status, $priorityLevel, $errorType, $description, $OS, $assigned,  $timeStart, $timeStop){
  ...
  if($name == ""){
    $errors[] = "<span class=\"errors\">Please enter your name.</span>";
  }
  if($email == ""){
    $errors[] = "<span class=\"errors\">Please enter a valid peoriachristian.org email address.</span>";
  }
  ...
  if($OS == "---SELECT ONE---"){
    $errors[] = "<span class=\"errors\">Please select a version of windows.</span>";
  }
  if($errorType == "---SELECT ONE---"){
    $errors[] = "<span class=\"errors\">Please select an error type.</span>";
  }
  if($description == ""){
    $errors[] =  "<span class=\"errors\">Please enter a description of the problem.</span>";
  }
  if($priorityLevel == "---SELECT ONE---"){
    $errors[] = "<span class=\"errors\">Please select a priority level.</span>";
  }
...
######################	
    ...
    request_email($db_connection, $email_action, $post_ID, $admin_email_notification_subject, $admin_email_list, $global_owner_email, $owner_email_beg_url, $owner);
...
#########################################################################################
#Purpose: Takes data from variables and formats it for entry to mySQL database via sql statements
function update_entry($db_connection, $owner, $name, $email, $assetNumber, $priorityLevel, $status, $timeStart, $errorType, $description, $OS, $assigned, $timeStop, $resolution, $RegID, $owner_email_beg_url, $admin_email_notification_subject, $global_owner_email)
{
...
########Submit data to the database via sql
	...
	//send email to request poster and to $owner's department
	echo "$admin_email_notification_subject<br />";
	request_email($db_connection, $email_action, $post_ID, $admin_email_notification_subject, $admin_email_list, $global_owner_email, $owner_email_beg_url, $owner);
...
function display_entry($db_connection, $owner, $view, $name, $email, $assetNumber, $priorityLevel, $status, $timeStart, $errorType, $description, $OS, $post_ID){
	...
	srqheader($owner); //create logo banner at top of page
        ...
        $scriptORecords = "SELECT RegID,         " .
                               ...
                               "  Resolution          " .
		                           "  FROM error_tracking " .
                               "  WHERE `Status`!='Resolved (Completed)' AND `Status`!='Closed (Denied)' AND ErrorType!='Internal Task' ORDER BY `error_tracking`.`RegID` ASC";
     ...
     // Create SQL statement to pull all closed or Resolved records
       ...
       $scriptCRecords = "SELECT RegID,         " .
                              ...
                              "  Resolution          " .
		                          "  FROM error_tracking " .
                              "  WHERE `Status`='Resolved (Completed)' OR `Status`='Closed (Denied)' ORDER BY `error_tracking`.`RegID` ASC";
     ...
     // Create SQL statement to pull all Intra-Dept Task records
       ...
       $scriptTRecords = "SELECT RegID,         " .
                              ...
                              "  Resolution          " .
		                          "  FROM error_tracking " .
                              "  WHERE ErrorType ='Internal Task' ORDER BY `error_tracking`.`RegID` ASC";
...
########################Create form to REQUEST A RECORD
    form_beg("actit_srq.php", "encmultipart/form-data", "mtdPOST");
    p_beg();
    table_beg();
    echo "<tr>";
    ...
    $script = "SELECT RegID,         " .
         ...
         "  Resolution " .
		 "  FROM error_tracking " .
         "  WHERE `Status`!='Resolved (Completed)' AND `Status`!='Closed (Denied)' ORDER BY `error_tracking`.`RegID` ASC";
...
# Fetch the contents of the requests
    ...
    /* execute query script statement */
...
#######################################################
    ...
    p_beg();
       ...
       frm_option("valWindows Vista","txtWindows Vista");	    
